...
Use this after the visitor has chosen products and a shipping destination. The backend expects a numeric `warehouse_id`; the runtime extracts it from the warehouse object when possible.
...
After `createOrder` succeeds, choose the post-order redirect yourself. The runtime creates the order only; it does not apply the Builder **After Submit Event** setting for you. See [HTML Mode checkout success paths](/html-mode-checkout-success-paths).
...
The runtime forwards the old Scalev analytics backend shapes for each provider. It adds the current page id, event source URL, referrer, and safe attribution values from browser cookies or URL parameters when available. For Facebook and TikTok events, `event_id` is generated automatically when omitted.
