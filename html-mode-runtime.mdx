...
Use it to render selected products, bundle options, and store-aware forms. Do not assume products or bundles that are not present in this payload.

## `Scalev.payments.enabled(username?)`

Returns enabled public payment methods for the business username.

```js
const payments = await Scalev.payments.enabled()
```

You can pass a username explicitly:

```js
const payments = await Scalev.payments.enabled("brand")
```

If omitted, the runtime uses the page business username. Use this when building a store-connected order form. Do not show payment methods that are not returned by this method or by the selected store context.
...
The runtime accepts common camelCase and snake_case order fields, including:

- Customer fields: `customerName`, `customerPhone`, `customerEmail`, `name`, `phone`, `address`
- Store and page fields: `storeUniqueId`, `store_unique_id`, `pageUniqueId`, `page_unique_id`
- Location and shipping fields: `locationId`, `postalCode`, `warehouseUniqueId`, `courierServiceId`
...
- Attribution-safe fields: UTM parameters, click ids, and analytics attribution gathered by the runtime
