...
Scalev supports business API keys for machine-authenticated server-to-server requests.

Business API keys are the simplest option when your own backend needs to call Scalev directly without running an OAuth install flow. They are intended for backend services, automation, internal tools, and other trusted machine integrations.

Business API keys authenticate authenticated business routes, primarily on the `/v3` API surface.
...
Most machine-authenticated business integrations should use `/v3` routes.

## API Key Types

### Secret Keys (`sk_...`)

Secret keys are full-access machine credentials for the business.
...
- secret keys do **not** bypass dashboard-only authorization rules
...
```bash
Authorization: Bearer rk_9z8y7x6w5v4u...
```

Important boundary:

- restricted keys cannot be granted dashboard-only scopes
...
From the dashboard, you can view the existing API keys for the business.
...
- you want to refresh a long-lived integration credential
- you are replacing credentials in downstream systems
...
Business API keys are sent in the `Authorization` header using bearer format:
...
## Where Business API Keys Work

Business API keys are for machine-authenticated business requests.

They work on routes that allow machine auth through business API keys.
...
Each business API key has a per-hour limit.
...
Business API keys are for authenticated API requests.
