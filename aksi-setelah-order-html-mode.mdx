...
| Langsung ke WhatsApp                | `direct_to_whatsapp`        | Buka WhatsApp memakai `order.handler_phone` dan `order.chat_message`. Nexus memilih handler dari assignment store atau halaman. Jika `handler_phone` tidak ada, arahkan ke **Self Hosted Orderan / Invoice** (`order_page`).                            |
...
Untuk halaman HTML Mode manual, tentukan ini secara eksplisit. Teruskan UTM, click, atau affiliate parameter saat kamu perlu menjaga continuity attribution. Jangan meneruskan nama customer, nomor telepon, alamat, atau data personal lain ke URL eksternal kecuali flow checkout membutuhkannya dan pembeli memang mengharapkannya.
