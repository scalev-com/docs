...
Runtime otomatis menetapkan checkout attempt yang idempotent untuk setiap submission. Jika browser kehilangan response, runtime akan memeriksa apakah Nexus sudah menyimpan order dan hanya melakukan satu retry yang aman jika diperlukan. Anda tidak perlu membuat, menyimpan, atau mengirim attempt ID di `payload`.
