...
<Update label="Juli 2026" tags={["API","Docs"]} description="Metode pembayaran canonical dan E-Payment Link">
  API order dan Storefront kini memakai satu `payment_method` canonical.
  Virtual account memakai value flat seperti `va_bca`; tidak ada field metode
  pembayaran sekunder dalam kontrak request atau response yang terdokumentasi.
  Store dapat menawarkan `payment_link`, dan checkout strict two-step dapat
  mengosongkan `payment_method` agar Scalev mengarahkan pembeli ke selector
  E-Payment Link. Response public order mengekspos pendapatan surcharge
  E-Payment Link yang dibayar customer sebagai `payment_link_income`. Webhook
  order mengekspos `payment_link_id` dan `is_from_payment_link` agar asal order
  tetap tersedia setelah `payment_method` order paid berubah menjadi metode
