...
The runtime automatically assigns an idempotent order-creation attempt to each submission. If the browser loses the response, it checks whether Nexus already committed the order and safely retries once only when needed. You do not need to create, persist, or pass an attempt ID in `payload`.
