...
For e-payment methods (`gopay`, `card`, `invoice`, any canonical `va_*` method, `qris`, `ovo`, `dana`, `shopeepay`, `linkaja`, `alfamart`, and `indomaret`), the hosted success page waits for `pg_payment_info` to appear by refetching the public order up to 15 times with a 1.5 second delay.
...
For canonical `va_*` methods, `bank_transfer`, `qris`, `alfamart`, and `indomaret`, it also shows a manual "check payment status" action.
