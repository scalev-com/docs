...
| Langsung ke WhatsApp                | `direct_to_whatsapp`        | Open WhatsApp using `order.handler_phone` and `order.chat_message`. Nexus chooses the handler from the store or page assignment. If `handler_phone` is missing, fall back to **Self Hosted Orderan / Invoice** (`order_page`).    |
...
For manual HTML Mode pages, be explicit. Forward UTM, click, or affiliate parameters when you need attribution continuity. Do not forward customer names, phone numbers, addresses, or other personal data to external URLs unless your checkout flow requires it and the buyer expects it.
